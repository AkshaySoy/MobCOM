...
--
-- Host: 127.0.0.1
-- Generation Time: Mar 07, 2021 at 03:01 AM
...
--
-- Stand-in structure for view `brand_models`
...
,`model_colour` varchar(255)
,`display_size` varchar(255)
,`display_resolution` varchar(255)
,`operating_system` varchar(100)
,`processor` varchar(255)
,`internal_storage` int(100)
,`ram` int(100)
,`memory_card_available` varchar(100)
,`primary_camera` varchar(255)
,`secondary_camera` varchar(255)
,`network_type` varchar(255)
,`battery_capacity` int(100)
,`mobile_width` int(100)
,`mobile_depth` int(100)
,`mobile_weight` int(100)
,`mobile_price` int(100)
,`mobile_availabilty` int(100)
);
...
--
-- Structure for view `brand_models`
--
DROP TABLE IF EXISTS `brand_models`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `brand_models`  AS SELECT `bm`.`brand_name` AS `brand_name`, count(`pm`.`brand_id`) AS `quantity` FROM (`product_master` `pm` join `brand_master` `bm`) WHERE `pm`.`brand_id` = `bm`.`brand_id` GROUP BY `pm`.`brand_id` ;
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `product_details`  AS SELECT `pm`.`product_id` AS `product_id`, `bm`.`brand_name` AS `brand_name`, `pm`.`product_desc` AS `product_desc`, `pm`.`model_number` AS `model_number`, `pm`.`model_name` AS `model_name`, `pm`.`model_colour` AS `model_colour`, `pm`.`display_size` AS `display_size`, `pm`.`display_resolution` AS `display_resolution`, `pm`.`operating_system` AS `operating_system`, `pm`.`processor` AS `processor`, `pm`.`internal_storage` AS `internal_storage`, `pm`.`ram` AS `ram`, `pm`.`memory_card_available` AS `memory_card_available`, `pm`.`primary_camera` AS `primary_camera`, `pm`.`secondary_camera` AS `secondary_camera`, `pm`.`network_type` AS `network_type`, `pm`.`battery_capacity` AS `battery_capacity`, `pm`.`mobile_width` AS `mobile_width`, `pm`.`mobile_depth` AS `mobile_depth`, `pm`.`mobile_weight` AS `mobile_weight`, `pm`.`mobile_price` AS `mobile_price`, `pm`.`mobile_availabilty` AS `mobile_availabilty` FROM (`product_master` `pm` join `brand_master` `bm`) WHERE `pm`.`brand_id` = `bm`.`brand_id` ;
